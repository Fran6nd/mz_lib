...
void _generate(mz_maze *maze_ptr, mz_position point,
  ...
  if (mz_is_pathable(maze_ptr, point)) {
    ...
    int i;
    for (i = 0; i < nb.n; i++) {
      if ((nb.paths[i].x == point.x && abs(nb.paths[i].y - point.y) == 2) ||
          (nb.paths[i].y == point.y && abs(nb.paths[i].x - point.x) == 2)) {
        mz_position tmp = nb.paths[0];
        nb.paths[0] = nb.paths[i];
        nb.paths[i] = tmp;
        nb.n--;
        break;
      }
